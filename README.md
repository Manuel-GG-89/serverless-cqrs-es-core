...
- It may even be the case that some component of the domain aggregate or its completeness cannot be implemented without state, and a Statefull instance is required (dockers, ES2 instances, or other similar ones). And it doesn't work. This case will require a monolithic approach to the development of the basic components (command, event and query handles) and there could be several internal applications on the server collaborating to process the aggregate tasks. But even in this case, the software architecture patterns must be respected, that is, communications between the command-handler will continue to emit events to the data bus, which will be destined for the event-handler of the same aggregate and any other interested in said event, the api will continue to be asynchronous via websocket, etc., the rules are the same.
