...
- It may even be the case that some component of the domain aggregate or its completeness cannot be implemented without state, and a Statefull instance is required (dockers, EC2 instances, or other similar ones). This case will require a monolithic approach to the development of the basic components (command, event and query handles) and there could be several internal applications on the server collaborating to process the aggregate tasks. But even in this case, the software architecture patterns must be respected, that is, communications between the command-handler and event-handler will continue bay events to the data bus, the clients api will continue to be asynchronous via websocket, etc., the rules are the same.
