...
Taking all of the above into consideration, a basic composition of a Domain Aggregate have, at least: 
  ...
  - A partitioning rule defined in the Event-Stores to generate snapshots (and backups) for each event-sore.Depending on the needs of the aggregate of domain, this can be configured to perform every 1 year, every 6 months, one month, a week or even one day if the domain aggregate manipulates demacious events.Everything will depend on the needs of the domain aggregate but, usually, snapchos are performed every 6 months or once a year.When performing a Snapshot, the system can be restarted from the final stored project Snapshot, which improves the speed of response in the reading and generation of new projections (starting with a base projection a more limited list of events to fold).In addition, this strategy can help generate system recoveries in case of failures, if necessary.
