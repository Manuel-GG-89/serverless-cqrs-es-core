...
Why Graphql as API ?
  ...
  - La operacion descrita se maneja completamente de forma asincronica, lo que no quiere decir que sea lenta. Al contrario, incluso en los casos relacionados con eventos provenientes de dispositivos IoT, telefonos inteligentes, trafico, etc, donde el volumen de eventos oncurrentes podria llegar a ser abrumados.
  -
  -  del aggregado de dominio (.. 'entidad') no suele tener muchos eventos, 100 ~ 150 eventos ,  en bases de datos clave-valor del proveedor (dynamodb) ya que posee un balance de caracteristicas de seguridad, auto escalamiento, sistema de particion incorrporado y mautomatizable para hacer busquedas por ID o filtrando por algun criterio)  
