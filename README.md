...
Another example: In passenger transport domain, by their nature, each signal recived from a bus, train, aeroplane, and any other, they are also stored as a succession of immutable events (GPS and sensors signals). And the pay system saves historical events each time that you arrives and uses it.
